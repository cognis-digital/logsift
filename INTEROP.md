...
How **logsift** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `logsift` lives in the **General OSINT / security tool** cluster.
...
| from | relation | to |
|---|---|---|
| `logsift` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis) |
| `logsift` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `logsift` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `logsift` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the general osint / security tool cluster.** `logsift` output feeds the next tool:
```bash
logsift ... --format json > out.json          # this tool's findings (see `logsift --help`)
...
logsift ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
logsift ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
logsift ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `logsift`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `logsift` slots
