...
[![PyPI](https://img.shields.io/pypi/v/cognis-logsift.svg)](https://pypi.org/project/cognis-logsift/)
[![CI](https://github.com/cognis-digital/logsift/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/logsift/actions)
...
LOGSIFT exists for one job — detect brute-force, spray, and anomalous auth events in logs — and does it without a SaaS bill or heavyweight setup.
...
```bash
logsift --version
logsift scan .                      # scan the current project
logsift scan . --format json
logsift scan . --fail-on high       # non-zero exit for CI gates
logsift mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **fail2ban**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`logsift` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
