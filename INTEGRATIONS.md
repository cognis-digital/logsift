...
**logsift** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `logsift` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
logsift ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
logsift ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
logsift ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `logsift` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="logsift") for rec in records]   # records = your JSON output
